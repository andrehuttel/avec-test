...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				$this->session->set_userdata('url_default', 'contacts');
	...
	public function historico_login($identity){
		$ip_address = $this->_prepare_ip($this->input->ip_address());
		return $this->db->insert('historico_login', array('ip_address' => $ip_address, 'login' => $identity, 'data_hora' => date('Y-m-d H:i:s')));
	}
	...
	**/
	public function delete_user($id)
	{
		...
		$this->db->delete('usuario_has_nivel_permissao', array('usuario_id' => $id));
	...
	public function login_remembered_user()
	{
		...
		//get the user
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column.', id, username, email, last_login, profissional_id, nivel_permissao_id')
						  ->join('usuario_has_nivel_permissao', 'usuario_has_nivel_permissao.usuario_id = '.$this->tables['users'].'.id')
		...
		if ($query->num_rows() == 1)
		{
			...
			$permissoes = $this->Auth_model->busca_permissao($user->nivel_permissao_id);

			foreach ($permissoes as $key => $perm) {
				$permissao[$perm->classe][$perm->metodo] = $perm->metodo;
			}
